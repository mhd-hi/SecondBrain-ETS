...
export const CommonErrorMessages = {
    ...
    PIPELINE_OPENAI_ERROR: 'Failed to parse with AI. Please try again later.',
...
export const PipelineErrorHandlers = {
    ...
    classifyError: (errorMessage: string): 'COURSE_EXISTS' | 'PLANETS_ERROR' | 'COURSE_NOT_FOUND' | 'PARSING_ERROR' | 'OPENAI_ERROR' | 'INVALID_FORMAT' | 'UNKNOWN' => {
        ...
        if (openaiPatterns.some(pattern => normalizedError.includes(pattern))) {
            return 'OPENAI_ERROR';
        }
    ...
    getSafeErrorMessage: (errorMessage: string): string => {
        ...
        switch (category) {
            ...
            case 'OPENAI_ERROR':
                return CommonErrorMessages.PIPELINE_OPENAI_ERROR;
            case 'PARSING_ERROR':
