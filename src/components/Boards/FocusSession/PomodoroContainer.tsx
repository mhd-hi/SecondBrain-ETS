...
interface StreakData {
    streakDays: number;
    lastCompletedPomodoroDate: string | null;
}
...
export const PomodoroContainer = ({ _onStartPomodoroWithTask }: PomodoroContainerProps) => {
    ...
    const [selectedDuration, setSelectedDuration] = useState(25); // Default 25 minutes    // Predefined duration options - includes classic and alternative Pomodoro variants
        ...
        const fetchStreak = async () => {
            try {
                ...
                if (streakResponse.ok) {
                    const streakData = await streakResponse.json() as StreakData;
            ...
            const fetchTasks = async () => {
                try {
                    ...
                    if (response.ok) {
                        ...
                        setTasks(tasksData);            // Auto-select the most urgent task (most overdue IN_PROGRESS task)
    ...
    const getCourseBadgeProps = (course: Course) => {
        ...
        <>
            <div className="border rounded-lg bg-muted/30 p-4 min-h-[320px] flex flex-col">
                ...
                <div className="space-y-3 flex-1">                    {/* Duration Selector */}
                            ...
                            <DropdownMenuContent>
                                {durationOptions.map((option) => (
                                    <DropdownMenuItem
                                        ...
                                        className={selectedDuration === option.value ? "bg-accent" : ""}
                                    >
                                        {option.label}
                                    </DropdownMenuItem>
                                ))}
                    ...
                    <div className="space-y-1">
                        <DropdownMenu>
                            ...
                            </DropdownMenuTrigger><DropdownMenuContent className="w-full">
                                {tasks.length === 0 ? (
